...
function detalleEstadia()
{
	var numero = urlParam('reg');

	
	$.ajax({
		url:"?view=consultas&tipo=detest&nr="+numero,
		...
		success:function(data)
		{
			if (data != '0') {
				...
				{
					/*Numero de solicitud*/
					$('#detEst1').html(data[i]['ids']);
					/*Datos de cliente*/
					$('#denombre1').val(data[i]['nombres']);
					$('#deapellido1').val(data[i]['apellidos']);
					$('#dedocumento1').val(data[i]['numerod']);
					$('#decelular1').val(data[i]['celular']);
					/*Datos de habitacion*/
					$('#dehabitacion1').val(data[i]['numeroh']);
					$('#detipo1').val(data[i]['tipoh']);
					$('#deingreso1').val(data[i]['fechaIni']);
					$('#dedias1').val(data[i]['dias']);
					$('#deprecio1').val(data[i]['total']);
					/*Datos de paga*/
					$('#deacuenta1').val(data[i]['acuenta']);
					$('#depagado1').val(data[i]['pagado']);
					$('#dedeuda1').val(data[i]['porpagar']);
					$('#detotal1').val(data[i]['total']);
				}
			}
		},
		complete:function() {
	...
	$.ajax({
		...
		success:function(data)
		{
			if (data == '1') {
				...
				$(location).attr('href','?view=menus&tipo=pagos');
