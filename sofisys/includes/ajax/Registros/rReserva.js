...
function calcDias()
{
  if ($.trim($('#f1').val()).length > 0) 
  {
    ...
    $.toast({
    heading: 'Error',
    text: 'No hay fechas de inicio ni fin seleccionadas!',
...
function restar () {
    ...
    if ($.trim(total).length > 0 && $.trim(acuenta).length) 
    {
        /* Esta es la resta. */
    ...
    else
    {
      $.toast({
        heading: 'Error',
        text: 'Debe calcular el total por estadia!',
        showHideTransition: 'slide',
        icon: 'error'
      })
    }
...
function alertDias()
{
  $.toast({
    heading: 'Error',
    text: 'La habitación <strong>'+p+'</strong> ya se encuentra ocupada',
